tableextension 31023106 "PTSS Purchase Header SGRP" extends "Purchase Header"
{
    ...
    begin
        ...
        IF InSeriesGroup(PostingDoc) THEN BEGIN
            //Debit Memo
            //IF "Debit Memo" THEN
            //    NoSeriesMgt.SetDefaultSeries("Posting No. Series", SeriesGroups."Posted Debit Memo");
            CASE PostingDoc OF
                1:
                    //Debit Memo
                    //IF NOT "Debit Memo" THEN
                    NoSeriesMgt.SetDefaultSeries("Posting No. Series", SeriesGroups."Posted Invoice");
                2:
                    NoSeriesMgt.SetDefaultSeries("Receiving No. Series", SeriesGroups."Shipment/Receipts");
                3:
                    NoSeriesMgt.SetDefaultSeries("Posting No. Series", SeriesGroups."Posted Credit Memo");
                4:
                    NoSeriesMgt.SetDefaultSeries("Return Shipment No. Series", SeriesGroups."Posted Ret. Receipts/Ship.");
                5:
    ...
    procedure GetPostingNoSeriesCodePT(PurchaseHeader: Record "Purchase Header"; Var PostingNos: Code[20])
    var
        UserSetup: Record "User Setup";
        SeriesGroups: Record "PTSS Series Groups SGRP";
        NoSeries: Record "No. Series";
    ...
    begin
        ...
        IF ("No. Series" <> '') AND (NoSeries.GET("No. Series")) AND (NoSeries."PTSS Series Group SGRP" <> '') THEN BEGIN
            SeriesGroups.GET(NoSeries."PTSS Series Group SGRP");
            IF SeriesGroups.Type = SeriesGroups.Type::Purchase THEN
                EXIT(TestPostingDocType(PostingDoc, SeriesGroups));
        END;
        EXIT(FALSE);
    end;
